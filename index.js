...
function initExpressionAvatar() {
    ...
    // HTML 설정 로드
    const settingsHtml = $(jQuery.parseHTML(getContext().extensionSettings.getSettingsHtml(extensionName)));
    $('#extensions_settings').append(settingsHtml);
    ...
    $('#expression_avatar_opacity').val(extension_settings[extensionName].originalOpacity);
    $('#expression_avatar_opacity_value').text(extension_settings[extensionName].originalOpacity);
    $('#expression_avatar_height').val(extension_settings[extensionName].avatarHeight);
    $('#expression_avatar_apply_to_user').prop('checked', extension_settings[extensionName].applyToUserMessages);
