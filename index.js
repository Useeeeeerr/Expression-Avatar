...
async function loadSettingsHTML() {
    try {
        // 스크립트 경로에서 확장 프로그램 폴더 이름 추출 시도
        const scriptElements = document.querySelectorAll("script");
        let extensionFolder = "Expression_Avatar"; // 기본값
        
        for (const element of scriptElements) {
            const src = element.getAttribute("src") || "";
            if (src.includes(extensionId)) {
                const match = src.match(/scripts\/extensions\/.*?\/([^\/]+)/);
                if (match && match[1]) {
                    extensionFolder = match[1];
                    break;
                }
            }
        }
        ...
        // HTML 파일 로드 시도
        const response = await fetch(`scripts/extensions/third-party/${extensionFolder}/index.html`);
        if (!response.ok) {
            console.error(`${extensionName}: Failed to load HTML from path: scripts/extensions/third-party/${extensionFolder}/index.html`);
            return;
        }
...
function setupEventListeners() {
    ...
    // MutationObserver 기반 메시지 처리 (이전 방식)
...
function processMessageElement(messageElement) {
    ...
    // AI 메시지인지 확인
    const isAiMessage = messageElement.classList.contains("mes_ai") || 
                        messageElement.classList.contains("ai_user") || 
                        messageElement.classList.contains("swipe_right");
...
async function loadExpressionImages(character) {
    ...
    try {
        // 먼저 새 API 엔드포인트 시도
        let response = await fetch(`/api/characters/get-expressions?name=${encodeURIComponent(character.name)}`);
        
        // 실패하면 이전 엔드포인트 시도
        if (!response.ok) {
            response = await fetch(`/getallexpression?name=${encodeURIComponent(character.name)}`);
            if (!response.ok) {
...
function restoreMessageAvatars() {
    const messages = document.querySelectorAll("#chat .mes_ai, #chat .ai_user, #chat .swipe_right");
    messages.forEach(messageElement => {
        const avatarElement = messageElement.querySelector(".avatar img");
        if (avatarElement && defaultAvatarImg) {
            avatarElement.src = defaultAvatarImg;
